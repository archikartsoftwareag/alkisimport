...
INSERT INTO ak_fs_fuellfl_umfang_nutz(fs_gml_id, fuellfl_umfang, wkb_geometry)
  SELECT fs_gml_id, fuellfl_umfang, wkb_fuellflaeche FROM (
    ...
    JOIN ax_tatsaechlichenutzung d ON d.gml_id = c.nutz_gml_id AND d.endet IS NOT DISTINCT FROM c.nutz_endet
    LEFT JOIN ak_fs_hist e ON e.gml_id = f.gml_id
    WHERE f.endet IS NULL OR e.gml_id IS NOT NULL
    GROUP BY f.gml_id) temp
  WHERE (fs_fl - nutz_fl) > (SELECT value FROM ak_alkis_options WHERE name = 'MinFüllfläche')::float;
